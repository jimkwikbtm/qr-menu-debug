        ...
        if (isset($seg[2])) {
            switch ($seg[2]) {
                case 'items':
                    ...
                    break;
                case 'history':
                    getOrderHistory($db, $orderUid);
                    break;
                case 'actions':
                    getOrderActions($db, $orderUid);
                    break;
                default:
                    throw new ApiException('Endpoint not found.', 404);
            }
...
SQL;
    ...
    try {
        ...
        sendJson(
            ['order_uid' => $orderUid, 'items' => $inserted],
            201,
...
// -----------------------------------------------------------------------------
// Enhanced Order Tracking Helper Functions
// -----------------------------------------------------------------------------

function useEnhancedSchema(PDO $db): bool {
    try {
        // Check if enhanced schema fields exist
        $stmt = $db->prepare("PRAGMA table_info(Orders)");
        $stmt->execute();
        $columns = $stmt->fetchAll(PDO::FETCH_COLUMN, 1);
        
        // Look for enhanced status fields or new columns
        return in_array('driver_name', $columns) || 
               in_array('served_at', $columns) ||
               in_array('pickup_counter', $columns);
    } catch (Exception $e) {
        return false;
    }
}

function buildOrderTypeInfo(PDO $db, array $order): array {
    $typeInfo = [
        'dine-in' => [
            'icon' => '🍽️',
            'type_description' => 'Dine-in Experience',
            'status_flow' => [
        ...
        'takeaway' => [
            'icon' => '🥤',
            'type_description' => 'Takeaway Order',
            'status_flow' => [
                ['status' => 'pending', 'icon' => 'fas fa-clock', 'label' => 'pending'],
                ['status' => 'confirmed', 'icon' => 'fas fa-check-circle', 'label' => 'confirmed'],
                ['status' => 'preparing', 'icon' => 'fas fa-utensils', 'label' => 'preparing'],
        ...
        'delivery' => [
            'icon' => '🚗',
            'type_description' => 'Home Delivery',
            'status_flow' => [
                ['status' => 'pending', 'icon' => 'fas fa-clock', 'label' => 'pending'],
                ['status' => 'confirmed', 'icon' => 'fas fa-check-circle', 'label' => 'confirmed'],
                ['status' => 'preparing', 'icon' => 'fas fa-utensils', 'label' => 'preparing'],
    ...
    $baseInfo = $typeInfo[$order['order_type']] ?? $typeInfo['dine-in'];
    
    // Add status-specific information
    $baseInfo['status_description'] = getStatusDescription($order);
    $baseInfo['detailed_status'] = getDetailedStatusDescription($db, $order);
    $baseInfo['estimated_time'] = getEstimatedTime($db, $order);
    
    // Add type-specific details if enhanced schema available
    if (useEnhancedSchema($db)) {
        $baseInfo['type_specific_details'] = getTypeSpecificDetails($db, $order);
    } else {
        $baseInfo['type_specific_details'] = simulateTypeSpecificDetails($order);
    }
    
    return $baseInfo;
}

function getStatusDescription(array $order): string {
    $descriptions = [
        'dine-in' => [
        ...
        'delivery' => [
            ...
            'preparing' => 'Chef is preparing your meal',
            'ready' => 'Order ready, awaiting driver',
            'completed' => 'Delivery completed, thank you'
        ]
    ];
    
    return $descriptions[$order['order_type']][$order['status']] ?? 'Status updated';
}

function getDetailedStatusDescription(PDO $db, array $order): string {
    $base = getStatusDescription($order);
    
    // Add context-specific details
    if ($order['order_type'] === 'dine-in' && !empty($order['table_identifier'])) {
        if ($order['status'] === 'preparing') {
            return $base . '. Your table ' . $order['table_identifier'] . ' is being prepared.';
        }
        if ($order['status'] === 'ready') {
            return $base . '. Your server will bring the food shortly.';
        }
    }
    
    if ($order['order_type'] === 'takeaway' && !empty($order['pickup_time'])) {
        return $base . '. Pickup time: ' . date('M j, Y g:i A', strtotime($order['pickup_time']));
    }
    
    if ($order['order_type'] === 'delivery' && !empty($order['estimated_delivery_time'])) {
        return $base . '. Estimated delivery: ' . date('M j, Y g:i A', strtotime($order['estimated_delivery_time']));
    }
    
    return $base;
}

function getEstimatedTime(PDO $db, array $order): ?string {
    switch ($order['order_type']) {
        case 'dine-in':
            if ($order['status'] === 'preparing') return '15-20 minutes';
            if ($order['status'] === 'ready') return 'Being served now';
            break;
        case 'takeaway':
            if (!empty($order['pickup_time'])) {
                return date('M j, Y g:i A', strtotime($order['pickup_time']));
            }
            if ($order['status'] === 'preparing') return '15-20 minutes';
        ...
        case 'delivery':
            if (!empty($order['estimated_delivery_time'])) {
                return date('M j, Y g:i A', strtotime($order['estimated_delivery_time']));
            }
            if ($order['status'] === 'preparing') return '20-30 minutes';
...
function getTypeSpecificDetails(PDO $db, array $order): array {
    $details = [];
    
    switch ($order['order_type']) {
        case 'dine-in':
            if (!empty($order['table_identifier'])) {
                $details['table_number'] = $order['table_identifier'];
            }
            if (!empty($order['served_at'])) {
                $details['served_at'] = $order['served_at'];
            }
            break;
        case 'takeaway':
            if (!empty($order['pickup_time'])) {
                $details['pickup_time'] = $order['pickup_time'];
            }
            if (!empty($order['pickup_counter'])) {
                $details['pickup_counter'] = $order['pickup_counter'];
            }
            break;
        case 'delivery':
            ...
            if (!empty($order['driver_name'])) {
                $details['driver_name'] = $order['driver_name'];
            }
            if (!empty($order['driver_phone'])) {
                // Mask phone number for privacy
                $details['driver_phone'] = substr($order['driver_phone'], 0, 2) . '**** ***' . substr($order['driver_phone'], -4);
            }
            if (!empty($order['estimated_delivery_time'])) {
                $details['estimated_delivery_time'] = $order['estimated_delivery_time'];
            }
            break;
    }
...
function simulateTypeSpecificDetails(array $order): array {
    $details = [];
    ...
    switch ($order['order_type']) {
        ...
        case 'takeaway':
            ...
            $details['pickup_counter'] = 'Main Counter'; // Default value
            break;
        case 'delivery':
            if (!empty($order['delivery_address'])) {
                $details['delivery_address'] = $order['delivery_address'];
            ...
            $details['driver_name'] = 'Delivery Partner'; // Default value
            break;
    }
...
function getAvailableActions(array $order): array {
    $actions = [];
    
    // Common action for all order types
        ...
        case 'dine-in':
            if (in_array($order['status'], ['confirmed', 'preparing'])) {
                $actions[] = [
                    ...
                    'icon' => 'fas fa-bell',
                    'description' => 'Request server assistance',
        ...
        case 'takeaway':
            if ($order['status'] === 'confirmed') {
                $actions[] = [
                    ...
                    'icon' => 'fas fa-clock',
                    'description' => 'Change your pickup time',
                    'requires_confirmation' => true
                ];
            }
            if ($order['status'] === 'ready') {
...
function buildOrderTimeline(PDO $db, array $order): array {
    $timeline = [];
    ...
    // Add simulated timeline based on current status and timestamps
    if (in_array($order['status'], ['confirmed', 'preparing', 'ready', 'served', 'completed'])) {
        $timeline[] = [
            'event_id' => 2,
            'status' => 'confirmed',
            'timestamp' => date('Y-m-d H:i:s', strtotime($order['created_at']) + 120), // +2 minutes
    ...
    if (in_array($order['status'], ['preparing', 'ready', 'served', 'completed'])) {
        $timeline[] = [
            'event_id' => 3,
            'status' => 'preparing',
            'timestamp' => date('Y-m-d H:i:s', strtotime($order['created_at']) + 300), // +5 minutes
            'description' => 'Chef started preparing your meal',
            'system_generated' => true
        ];
    }
    
    // Add order type specific timeline events
    if ($order['order_type'] === 'dine-in' && in_array($order['status'], ['ready', 'served', 'completed'])) {
        $timeline[] = [
            'event_id' => 4,
            'status' => 'ready',
            'timestamp' => date('Y-m-d H:i:s', strtotime($order['created_at']) + 900), // +15 minutes
            'description' => 'Food ready for service',
            'system_generated' => true
        ];
    }
    
    if ($order['order_type'] === 'takeaway' && in_array($order['status'], ['ready', 'completed'])) {
        $timeline[] = [
            'event_id' => 4,
            'status' => 'ready',
            'timestamp' => date('Y-m-d H:i:s', strtotime($order['created_at']) + 900), // +15 minutes
    ...
    if ($order['order_type'] === 'delivery' && $order['status'] === 'out_for_delivery') {
        $timeline[] = [
            ...
            'timestamp' => date('Y-m-d H:i:s', strtotime($order['created_at']) + 1800), // +30 minutes
            'description' => 'Driver out for delivery',
            'system_generated' => true
        ];
    }
...
function getOrder(PDO $db, string $orderUid)
{
    ...
    $enhancedSchema = useEnhancedSchema($db);
    $orderTypeInfo = buildOrderTypeInfo($db, $order);
    $timeline = buildOrderTimeline($db, $order);
    $availableActions = getAvailableActions($order);
    
    $enhancedResponse = [
        'order' => $order,
        'enhanced' => [
            'type_specific' => $orderTypeInfo,
            'timeline' => $timeline,
            'available_actions' => $availableActions
        ],
        'schema_version' => $enhancedSchema ? 'enhanced' : 'legacy'
    ];
    
    sendJson($enhancedResponse);
}
...
function getOrderHistory(PDO $db, string $orderUid)
{
    $order = getCachedData("order_$orderUid", function() use ($db, $orderUid) {
        $stmt = $db->prepare('SELECT order_id, order_type, status, created_at FROM Orders WHERE order_uid = ?');
        $stmt->execute([$orderUid]);
        return $stmt->fetch(PDO::FETCH_ASSOC);
    });
...
function getOrderActions(PDO $db, string $orderUid)
{
    $order = getCachedData("order_$orderUid", function() use ($db, $orderUid) {
        $stmt = $db->prepare('SELECT order_type, status FROM Orders WHERE order_uid = ?');
        $stmt->execute([$orderUid]);
        return $stmt->fetch(PDO::FETCH_ASSOC);
    });
